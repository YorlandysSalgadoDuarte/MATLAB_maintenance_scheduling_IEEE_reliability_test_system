function [time, x, fval, exitflag, output] = particle_swarm_optimization()
...
options = optimoptions('particleswarm', 'Display', 'iter', 'UseParallel', true, 'Vectorized', 'off', 'PlotFcn', @pswplotbestf, 'OutputFcn', @savingSchedulingParticleSwarm); % setting of the pso algorithm
rng("default") % control random number generation
[x, fval, exitflag, output] = particleswarm(objectiveFunction, nvars, LB, UB, options); % pso algorithm
